...
def getPlexTracks(plex: PlexServer, spotifyTracks: []) -> List[Track]:
    ...
    for spotifyTrack in spotifyTracks:
        ...
        logging.info(
            f"Searching Plex for: {track['name']} by {track['artists'][0]['name']}"
        )
        ...
        if len(musicTracks) > 0:
            ...
            if len(plexMusic) > 0:
                logging.info(
                    f"Found Plex Song: {track['name']} by {track['artists'][0]['name']}"
                )
                plexTracks.append(plexMusic[0])
            else:
                logging.info(
                    f"Could not find song on Plex: {track['name']} by {track['artists'][0]['name']}"
                )
    return plexTracks
...
def runSync(plex: PlexServer, sp: spotipy.Spotify, spotifyURIs: []):
    ...
    for spotifyUriParts in spotifyURIs:
        if (
            ...
            logging.info(
                f"Getting playlist {spotifyUriParts['user']} from user {spotifyUriParts['playlist']}"
            )
            playlists.append(
                getSpotifyPlaylist(
